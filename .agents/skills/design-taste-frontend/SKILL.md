...
### 0.E Custom Project Override (InsuStream Portal Cozy Minimalist Directive)
* **Design Directive:** Redesign the user interface using a Cozy Minimalist aesthetic inspired by vochi.xyz. Use a soft, warm cream-gray background (`#f5f5f2`), clean white paper cards with ultra-thin hairline borders (`#e3e3dc`), a classy deep forest/sage green primary color (`#2b5c43`), and dual-font pairings featuring elegant italic serif highlights (e.g., `Playfair Display` for subheadings and emphasis) alongside clean modern UI typography (`Plus Jakarta Sans`). Remove all dark-mode and neon/glow properties to maintain a pure, comforting, and bespoke light-mode layout.
