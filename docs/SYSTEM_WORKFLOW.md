...
4.  **Insert Database**: Chỉ các bản ghi được xác nhận là mới (không trùng lặp) mới được chèn vào bảng `stgContractObjectOffline` thuộc database `affina_staging`.
...
*   **Debezium Connector (`mysql-source-connector`)**: Giám sát nhật ký giao dịch (binlog) trên database sản xuất `affina_sale` đối với các bảng liên quan đến hợp đồng (`contract`, `contractObject`, `claim`...).
*   **Kafka Topics (`source.affina_sale.*`)**: Nhận các sự kiện thay đổi dữ liệu từ Debezium.
*   **CDC Consumer (`server_consumer`)**:
    ...
    *   Ghi đè hoặc cập nhật trực tiếp vào cơ sở dữ liệu `affina_staging` (các bảng `stgContract`, `stgContractObject*`).
...
*   **Debezium Connector (`mysql-staging-connector`)**: Theo dõi các thay đổi trên database `affina_staging` (bao gồm cả dữ liệu Online do CDC Consumer ghi và dữ liệu Offline do Portal tải lên).
*   **Kafka Topics (`staging.affina_staging.*`)**: Phân phối luồng dữ liệu staging.
*   **Streaming ETL Consumer (`affina_streaming_etl`)**:
    ...
    *   **UPSERT** dữ liệu vào bảng rộng `affina_reporting.contract`.
*   **Profiling Consumer (`affina_profiling_consumer`)**:
    ...
    *   **UPSERT** vào bảng phân tích `affina_reporting.profiling_analysis`.

### 3.3. Downstream Notification (Event Publisher)
*   **Event Publisher (`affina_event_publisher`)**:
    *   Quét định kỳ các bảng dữ liệu `affina_staging` và `affina_reporting` để phát hiện các sự kiện thêm mới hoặc cập nhật hợp đồng/bồi thường.
...
1.  **Xây Dựng Cache (Phía `cdc_reporting`)**:
    *   Script `redis_cache_builder.py` quét toàn bộ dữ liệu hợp đồng online đang có trong database `affina_staging` (của tất cả các loại bảo hiểm).
...
2.  **Kiểm Tra Khi Upload Excel (Phía `portal_backend`)**:
    ...
    *   Gửi lệnh kiểm tra nhanh tồn tại (`EXISTS`) lên Redis:
        ...
        *   **Nếu EXISTS = False**: Bản ghi chưa tồn tại. Portal sẽ thực hiện chèn bản ghi này vào cơ sở dữ liệu `affina_staging.stgContractObjectOffline`.
...
| **CDC Consumer** | `cdc_reporting` | Đọc dữ liệu thô từ Kafka, định dạng kiểu dữ liệu và ghi vào Staging. | Kafka `source.*` topics | Bảng staging tại `affina_staging` |
| **Streaming ETL** | `cdc_reporting` | Hợp nhất dữ liệu chi tiết của các loại bảo hiểm thành một Wide Table báo cáo. | Kafka `staging.*` topics | Bảng `affina_reporting.contract` |
| **Profiling Consumer**| `cdc_reporting` | Tính toán các chỉ số phân tích người dùng, hồ sơ bệnh án thời gian thực. | Kafka `staging.*` topics | Bảng `affina_reporting.profiling_analysis` |
